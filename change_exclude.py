...
excluded_columns = ["LastUpdated"]  # ⛔ Columns to ignore when comparing
...
# Ensure excluded columns exist in both
for col in excluded_columns:
    if col not in df1.columns or col not in df2.columns:
        raise ValueError(f"Excluded column '{col}' not found in both files.")

# Drop excluded columns for comparison
df1_compare = df1.drop(columns=excluded_columns)
df2_compare = df2.drop(columns=excluded_columns)

# Reindex both to align all IDs
all_ids = df1_compare.index.union(df2_compare.index)
df1_compare = df1_compare.reindex(all_ids)
df2_compare = df2_compare.reindex(all_ids)

# === STEP 4: Compare with null-safe fill ===
df1_filled = df1_compare.fillna("__NA__")
df2_filled = df2_compare.fillna("__NA__")
diff_df = df1_filled.ne(df2_filled)

# === STEP 5: Reload second Excel file ===
...
# === STEP 9: Compare rows and write summary ===
for idx in diff_df.index:
    ...
    if is_new_row:
        ...
        # Only consider non-excluded columns for cell-level changes
        row_diff = diff_df.loc[idx]
        for col_name, changed in row_diff.items():
            if changed and col_name in headers and col_name not in excluded_columns:
